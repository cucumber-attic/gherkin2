%%{
  ...
  FEATURE = 'תכונה:' >start_keyword %end_keyword; 

  # BACKGROUND = 'Background:' >start_keyword %end_keyword;
  BACKGROUND = 'רקע:' >start_keyword %end_keyword;

  # SCENARIO = 'Scenario:' >start_keyword %end_keyword;
  SCENARIO = 'תרחיש:' >start_keyword %end_keyword;

  # STEP = ('Given ' | 'When ' | 'And ' | 'Then ' | 'But ') >start_keyword %end_keyword;
  STEP = ('בהינתן ' | 'כאשר ' | 'וגם ' | 'אז|אזי ' | 'אבל ') >start_keyword %end_keyword;
