%%{
  ...
  FEATURE = 'خاصية:' >start_keyword %end_keyword; 

  # BACKGROUND = 'Background:' >start_keyword %end_keyword;
  BACKGROUND = 'الخلفية:' >start_keyword %end_keyword;

  # SCENARIO = 'Scenario:' >start_keyword %end_keyword;
  SCENARIO = 'سيناريو:' >start_keyword %end_keyword;

  # STEP = ('Given ' | 'When ' | 'And ' | 'Then ' | 'But ') >start_keyword %end_keyword;
  STEP = ('بفرض ' | 'متى|عندما ' | 'و ' | 'اذاً|ثم ' | 'لكن ') >start_keyword %end_keyword;
