%%{
  ...
  FEATURE = 'Tính năng:' >start_keyword %end_keyword; 

  # BACKGROUND = 'Background:' >start_keyword %end_keyword;
  BACKGROUND = 'Bối cảnh:' >start_keyword %end_keyword;

  # SCENARIO = 'Scenario:' >start_keyword %end_keyword;
  SCENARIO = 'Tình huống|Kịch bản:' >start_keyword %end_keyword;

  # STEP = ('Given ' | 'When ' | 'And ' | 'Then ' | 'But ') >start_keyword %end_keyword;
  STEP = ('Biết|Cho ' | 'Khi ' | 'Và ' | 'Thì ' | 'Nhưng ') >start_keyword %end_keyword;
