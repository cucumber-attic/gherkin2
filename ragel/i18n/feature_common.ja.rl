%%{
  ...
  FEATURE = 'フィーチャ|機能:' >start_keyword %end_keyword; 

  # BACKGROUND = 'Background:' >start_keyword %end_keyword;
  BACKGROUND = '背景:' >start_keyword %end_keyword;

  # SCENARIO = 'Scenario:' >start_keyword %end_keyword;
  SCENARIO = 'シナリオ:' >start_keyword %end_keyword;

  # STEP = ('Given ' | 'When ' | 'And ' | 'Then ' | 'But ') >start_keyword %end_keyword;
  STEP = ('前提 ' | 'もし ' | 'かつ ' | 'ならば ' | 'しかし|但し ') >start_keyword %end_keyword;
