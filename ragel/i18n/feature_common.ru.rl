%%{
  ...
  FEATURE = 'Функционал:' >start_keyword %end_keyword; 

  # BACKGROUND = 'Background:' >start_keyword %end_keyword;
  BACKGROUND = 'Предыстория:' >start_keyword %end_keyword;

  # SCENARIO = 'Scenario:' >start_keyword %end_keyword;
  SCENARIO = 'Сценарий:' >start_keyword %end_keyword;

  # STEP = ('Given ' | 'When ' | 'And ' | 'Then ' | 'But ') >start_keyword %end_keyword;
  STEP = ('Допустим ' | 'Если ' | 'И|К тому же ' | 'То ' | 'Но|А ') >start_keyword %end_keyword;
